...
namespace Karya3.Scripts.UI
{
    ...
    /// </summary>
    public partial class BeguGanjangIntroScene : Control
    {
        ...
        private void InitializeUI()
        {
            ...
            _textLabel.Size = new Vector2(GetViewportRect().Size.X - 100, GetViewportRect().Size.Y - 100);
        ...
        private void OnTypeTimerTimeout()
        {
            if (_currentCharIndex < _storyFrames[_currentFrame].Length)
            {
                ...
                // Increase pulse intensity briefly when typing
                _pulseIntensity = Math.Min(_pulseIntensity + 0.1f, 0.3f);
        ...
        private void AdvanceToNextFrame()
        {
            ...
            if (_currentFrame < _storyFrames.Length)
            {
                ...
                FadeOutCurrentFrame(() => StartFrame(_currentFrame));
            }
            else
            {
                // End the intro sequence
                FadeOutCurrentFrame(EndIntroSequence);
            }
        }
        ...
        private void PlayFrameAmbientSound(int frameIndex)
        {
            ...
            switch (frameIndex)
            {
                ...
                case 1:
                    ...
                    // _ambientSound.Stream = ResourceLoader.Load<AudioStream>("res://path_to_breathing_sound.wav");
                    break;
            }
        ...
        private void AnimateShadowFigure(int frameIndex)
        {
            ...
            switch (frameIndex)
            {
                case 0:
                    ...
                    _shadowFigure.Position = new Vector2(GetViewportRect().Size.X * 0.8f, GetViewportRect().Size.Y * 0.2f);
                    
                    _shadowTween.TweenProperty(_shadowFigure, "modulate:a", 0.1f, 5.0f);
                    _shadowTween.TweenProperty(_shadowFigure, "scale", new Vector2(0.6f, 0.6f), 15.0f);
                    _shadowTween.Parallel().TweenProperty(_shadowFigure, "position:y", GetViewportRect().Size.Y * 0.25f, 15.0f);
                    break;
                    
                case 1:
                    // Frame 2: Shadow grows larger and comes closer
                    _shadowFigure.Scale = new Vector2(0.7f, 0.7f);
                    _shadowFigure.Position = new Vector2(GetViewportRect().Size.X * 0.5f, GetViewportRect().Size.Y * 0.3f);
                    
                    _shadowTween.TweenProperty(_shadowFigure, "modulate:a", 0.2f, 3.0f);
                    _shadowTween.TweenProperty(_shadowFigure, "scale", new Vector2(1.2f, 1.2f), 20.0f);
                    _shadowTween.Parallel().TweenProperty(_shadowFigure, "position:y", GetViewportRect().Size.Y * 0.4f, 20.0f);
                    ...
                    _shadowTween.TweenProperty(_shadowFigure, "scale", new Vector2(2.0f, 2.0f), 0.5f);
                    _shadowTween.TweenProperty(_shadowFigure, "position:y", GetViewportRect().Size.Y * 0.6f, 0.5f);
                    break;
            }
        }
        ...
        {
            // Fade to black
            ColorRect blackOverlay = new ColorRect
            {
            ...
            fadeTween.TweenProperty(blackOverlay, "color:a", 1.0f, 2.0f);
            
            // When fade is complete, transition to the main game
            fadeTween.Finished += () => 
            {
                // Play final sound effect (distant scream)
                // _effectSound.Stream = ResourceLoader.Load<AudioStream>("res://path_to_scream_sound.wav");
                _effectSound.Play();
                
                // Wait for sound to finish, then transition to main game
                GetTree().CreateTimer(2.0f).Timeout += () => 
                {
                    // Transition to main game scene
                    // GetTree().ChangeSceneToFile("res://path_to_main_game_scene.tscn");
                    
                    // For testing, just queue free this scene
                    QueueFree();
                };
            };
        }
